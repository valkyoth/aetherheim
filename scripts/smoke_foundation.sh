...
printf '%s\n' "$help_output" | rg -q -F -x "Aetherheim $expected_version"
...
printf '%s\n' "$doctor_output" | rg -q -F -x "version=$expected_version"
printf '%s\n' "$doctor_output" | rg -q '^publishing=disabled$'
printf '%s\n' "$doctor_output" | rg -q '^status=foundation-only$'
...
printf '%s\n' "$unknown_output" | rg -q '^unknown command: definitely-not-a-command$'
